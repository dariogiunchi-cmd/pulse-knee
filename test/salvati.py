...
with sync_playwright() as p:
    ...
    N=numeri(pg)
    for n in N[:3]: pg.evaluate(f"toggleSave(null,{n})")
    pg.wait_for_timeout(200)
    chk("3 salvati oggi", pg.evaluate("savedList().length")==3)
    ...
    chk("3 salvati ancora presenti domani", pg.evaluate("savedList().length")==3)
    chk("le schede salvate si vedono", pg.locator("#savedList .item").count()==3, pg.locator("#savedList .item").count())
    chk("contatore corretto", pg.inner_text("#savedCount")=="3")
    ...
    pg.wait_for_timeout(200)
    chk("si può togliere un salvato di ieri", pg.evaluate("savedList().length")==2)
